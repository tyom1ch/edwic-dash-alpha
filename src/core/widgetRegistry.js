...
export const WIDGET_REGISTRY = [
  {
    ...
    topicFields: ['state_topic', 'unit_of_measurement'],
    getTopicMappings: (config) => ({ value: config.state_topic }),
  },
  {
    type: 'switch',
    label: 'Перемикач (ON/OFF)',
    component: SwitchComponent,
    topicFields: ['state_topic', 'command_topic', 'payload_on', 'payload_off'],
    getTopicMappings: (config) => ({ value: config.state_topic }),
    getCommandMappings: (config) => ({ default: config.command_topic }),
  },
