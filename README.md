...
Нові віджети
✨ **Нові можливості**
- Додано віджети `BinarySensor`, `Light`, `Fan`, `Cover`, `Number`.
- Додано можливість вибору схеми для віджета `Light` (json/default).
- Для віджету `Number` додано режими відображення (slider/box).

♻️ **Зміни**
- Перероблено конфігурацію полів для віджетів з використанням keys-аліасів.
- Додано підтримку `value_template` у `SensorComponent` для заокруглення значень.
- Змінено логіку відображення стану у віджеті `SwitchComponent`.
- Віджет `ClimateComponent` тепер використовує `variant` для визначення необхідних полів.
- В UI додано adaptive text scale.

🐛 **Виправлення**
- Виправлено обчислення `baseFontSize` в `SensorComponent`.

🧹 **Внутрішні зміни**
- Оптимізовано обробку топіків за допомогою функції `resolveTopic`.
- Додано документацію `description.md`.
- Видалено зайві коментарі та логіку.
- Оновлено README.md.
<!-- CHANGELOG END -->
